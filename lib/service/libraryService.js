...
const libraryService = {
    ...
    search_library_name: async (req, res) => {
        ...
        logger.info(`검색어 입력값 - 도서관명: ${title} - 지역: ${region} - 페이지: ${pageNo}`);
        ...
        // logger.info('페이징된 데이터 : ' + JSON.stringify(paginatedData));
        ...
        logger.error('Spring 서버와의 통신 에러 : ' + error);
    ...
    getLoanAvailable: async (req, res) => {
        ...
        try {
            if (libraryCode) {
                ...
                logger.info(`Location Data - 위도 : ${latitude} - 경도: ${longitude}`);
    ...
    library_detail: async (req, res) => {
        try {
            ...
            logger.info('LIB DETAIL : ' + JSON.stringify(libDetail));

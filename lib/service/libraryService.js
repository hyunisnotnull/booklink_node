...
const libraryService = {
    ...
    search_library_name: async (req, res) => {
        ...
        console.log('검색어 입력값:', { title, region });
        ...
        console.log('값:', springResponse.data);
    
        // Spring 서버의 응답을 그대로 클라이언트로 전달
        res.status(200).json(springResponse.data);
        } catch (error) {
    ...
    getLoanAvailable: async (req, res) => {
        const { bookID, latitude, longitude } = req.query;

        console.log('bookID:::',bookID);
        console.log('Location Data:', { latitude, longitude });

        try {
            // 스프링 서버로 위치 정보와 함께 쿼리 파라미터 전송
            ...
            console.log('libraryCodes :::', libraryCodes);
            ...
            for (const library of libraryCodes) {
                ...
                try {
                    ...
                    console.log('tq', response.data.response.result);

                    if (hasBook === 'Y') { // 소장 도서만 처리 , 다 들고 올거면 if문 삭제
            ...
            console.log('loanAvailableResults :::', loanAvailableResults);

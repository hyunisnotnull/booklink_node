...
const userService = {
    ...
    delete: (req, res) => {
        logger.info(`DELETE USER ID : ${req.params.userId}`)
        const u_ID = req.params.userId
    ...
    google: (req, res) => {
        logger.warn(`GOOGLE CODE : ${req.body.code}`)
        ...
        .then(data => {
            ...
            if(data.u_ID === null) {
                ...
                return fetch(`${process.env.SPRING_URL}/user/addUser`,{
                    method: "POST",
                    body: JSON.stringify({
                      u_ID: email,
                    ...
                    .then(() => {
                        // 첫 회원가입시 알림
                        const token = jwt.sign({ userId: email, alert_message: "회원 정보 수정이 필요합니다." }, process.env.SECURITY_KEY, { expiresIn: 60 * 30 });
                        res.cookie('token', token);
                        return res.json({ userId: email, alert_message: "회원 정보 수정이 필요합니다." });
                    });
            } else if(data.u_ID === email && data.u_SNS_ID === null)  {
                return fetch(`${process.env.SPRING_URL}/user/modifyUser`,{
                    method: "POST",
                    ...
                    .then(() => {
                        ...
                        const token = jwt.sign(payload, process.env.SECURITY_KEY, { expiresIn: 60 * 30 });
                        res.cookie('token', token);
    ...
    addWishBook: async (req, res) => {
        ...
        logger.info('Wish Book DTO : ' + WishBookDto);
    ...
    cancleWishBook: async (req, res) => {
        ...
        logger.info(`CANCEL WISH BOOK - USER : ${W_U_ID} - ISBN : ${W_ISBN13}`);
    ...
    readWishBook: async (req, res) => {
        ...
        logger.info(`READ WISH BOOK -  USER : ${W_U_ID} - ISBN : ${W_ISBN13} - READ : ${W_B_READ}`);
    
        if (W_B_READ !== 0 && W_B_READ !== 1) {
            return res.status(400).json({ message: '읽음 상태 값 오류' });
        }
        ...
        try {
            ...
            const readStatus = W_B_READ === 1 ? '읽음' : '읽지 않음';
            logger.info('res:::' + response.data);
            res.status(200).json({ message: `도서가 [${readStatus}] 처리 됐습니다.`, data: response.data });
    ...
    addWishLib: async (req, res) => {
        ...
        logger.info('Favorite Library Dto : ' + WishLibraryDto);
    ...
    cancleWishLib: async (req, res) => {
        ...
        logger.info(`CANCEL WISH LIB - USER : ${ML_U_ID} - LIB_CODE : ${ML_L_CODE}`);

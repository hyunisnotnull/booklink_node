...
const bookService = {
    ...
    search_book: async (req, res) => {
      const { title, author, publisher, isbn13, sort, order, pageNo } = req.query;
        logger.info('검색어 입력값:', { title, author, publisher, isbn13, sort, order, pageNo });
        try {
          ...
          logger.info('book ::: ', response.data.response);
    ...
    search_library: async (req, res) => {
      ...
      logger.info('검색어 입력값:', { title, isbn, region, pageNo });
      ...
      try {
        ...
        if (!isbn && title) {
          ...
          logger.info('bookResponse :', bookResponse.data.response);
          ...
          logger.info('searchBooks :::', searchBooks);

...
const bookService = {
    detail: async (req, res) => {
        try {
            ...
            // console.log('BOOK DETAIL:::', bookDetail.detail[0].book);
            ...
            let bookRelated = bookRelatedResponse.data.response.docs;
            ...
            const limitedRelatedBooks = bookRelated.slice(0, 8);

            // 선택된 도서 전달
            return res.json({ bookDetail, bookRelated: limitedRelatedBooks });
        } catch (error) {
    ...
    library_detail: async (req, res) => {
      try {
          ...
          console.log('LIB DETAIL:::', libDetail);
          
          // 도서관 신착 도서 API 호출
          const newArrivalBookResponse = await axios.get(`${process.env.BOOK_API_URL}newArrivalBook?authKey=${process.env.BOOK_API_KEY}&libCode=${libCode}&format=json`);
          const newArrivalBook = newArrivalBookResponse.data?.response?.docs || [];

          console.log('LIB BOOKS:::', newArrivalBook);

          // 선택된 도서 전달
          return res.json({ libDetail, newArrivalBook });
      } catch (error) {
